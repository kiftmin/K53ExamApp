...
import { Pencil, Trash2, Plus, Database, Power, PowerOff } from "lucide-react";
...
export default function SourceMaintenance({ isOpen, onClose }: SourceMaintenanceProps) {
    ...
    const createMutation = useMutation({
        mutationFn: async (name: string) => {
            return apiRequest("POST", "/api/sources", { name, is_active: true });
        },
        onSuccess: () => {
    ...
    const toggleActiveMutation = useMutation({
        mutationFn: async ({ id, isActive }: { id: number; isActive: boolean }) => {
            return apiRequest("PATCH", `/api/sources/${id}/active`, { isActive });
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ["/api/sources"] });
            queryClient.invalidateQueries({ queryKey: ["/api/sources/active"] });
                ...
                <div className="border rounded-md overflow-hidden">
                    <Table>
                        ...
                        <TableBody>
                            {isLoading ? (
                                <TableRow>
                                    <TableCell colSpan={3} className="text-center py-4 text-neutral-500">Loading sources...</TableCell>
                                </TableRow>
                            ) : sources?.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={3} className="text-center py-4 text-neutral-500">No sources found.</TableCell>
                                </TableRow>
                            ) : (
                                sources?.map((source) => (
